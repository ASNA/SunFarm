...
// Migrated source location: library NUTSNBOLTS, file QRPGSRC, member ORDDTLINQ
...
namespace SunFarm.Customers
{
    ...
    [ProgramEntry("_ENTRY")]
    public partial class Orddtlinq : ASNA.QSys.Runtime.JobSupport.Program
    {
...
#region Constructor and Dispose 
        public Orddtlinq()
        {
            ...
            ORDERHDRL2.Open(Job.Database, AccessMode.RWCD, false, false, ServerCursors.Default);
            ORDERHDRL2_ReadOnly.Overrider = Job;
            ORDERHDRL2_ReadOnly.Open(Job.Database, AccessMode.Read, false, false, ServerCursors.Default);
            ORDERLINL2.Overrider = Job;
            ORDERLINL2.Open(Job.Database, AccessMode.RWCD, false, false, ServerCursors.Default);
        }
        ...
        void RcdUpdate()
        {
            ...
            while (!(bool)_IN[12])
            {
                ...
                if ((bool)_IN[4])
                {
                    pSelOnly = "Y";
                    // Prompting?
                    DynamicCaller_.CallD("SunFarm.Customers.ITEMINQ", out _LR, ref pSelOnly, ref pItmNbr);
                    SITMNBR = pItmNbr;
...
#region Entry and activation methods for *ENTRY
        ...
        {
            return new Orddtlinq();
        }

        int _parms;

        void __ENTRY(out Indicator __inLR, bool _isNew, ref FixedDecimal<_9, _0> _pCUSTNO, ref FixedDecimal<_9, _0> _pORDNBR, ref FixedString<_1> _pDeleteAll)
        {
            int _pc_parms = 3;
            bool _cleanup = true;
        ...
        public static void _ENTRY(ICaller _caller, out Indicator __inLR, ref FixedDecimal<_9, _0> pCUSTNO, ref FixedDecimal<_9, _0> pORDNBR, ref FixedString<_1> pDeleteAll)
        {
            ...
            bool _isNew = false;
            Orddtlinq _instance = null;
            __inLR = '0';
            _instance = _manager.GetInstance(typeof(Orddtlinq), _classFactory, _caller, out _isNew) as Orddtlinq;
            try
            {
                _instance.__ENTRY(out __inLR, _isNew, ref pCUSTNO, ref pORDNBR, ref pDeleteAll);
            }
            catch
            {
                __inLR = '1';
